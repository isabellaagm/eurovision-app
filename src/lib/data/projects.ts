...
// Versão final com a query corrigida usando alias para garantir o formato do objeto.
...
export async function listProjects(): Promise<ListProjectsResult> {
  ...
  if (!supabase) { return { status: 200, projects: mockProjects, meta: { source: "mock", warning: "Supabase não configurado." } }; }

  try {
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) { return { status: 401, error: "Usuário não autenticado." }; }

    const { data, error } = await supabase
      .from("projects")
      .select(
        `
        id, name, description, status, gerencia, created_at, updated_at, last_modified_by_user_id, budget_allocated,
  user_profiles:last_modified_by_user_id ( id, full_name, avatar_url ),
  project_participants (
    user_id,
    user_profiles (id, full_name, avatar_url, job_title)
  )
        `
      )
    ...
    if (error) {
      ...
      return { status: 500, error: error.message || DEFAULT_ERROR, };
    }
    return { status: 200, projects: (data as Project[]) ?? [], meta: { source: "supabase" }, };
  } catch (err) {
    console.error("Erro inesperado ao listar projetos:", err);
    return { status: 500, error: err instanceof Error ? err.message : DEFAULT_ERROR, };
  }
}
...
export async function fetchProjectById(id: string): Promise<GetProjectResult> {
  ...
  if (!supabase) {
    ...
    if (!project) { return { status: 404, error: "Projeto não encontrado." }; }
    return { status: 200, project, meta: { source: "mock", warning: "Supabase não configurado." } };
  }
  ...
  project_participants (
    ...
    if (error) {
      if (error.code === "PGRST116") { return { status: 404, error: "Projeto não encontrado." }; }
      console.error(`Erro Supabase ao buscar projeto ${id}:`, error);
      return { status: 500, error: error.message || DEFAULT_ERROR, };
    }
    return { status: 200, project: data as Project, meta: { source: "supabase" }, };
  } catch (err) {
...
// O restante das funções (createProject, etc.) não precisam de alteração.
